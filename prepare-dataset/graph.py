...
class Vertex(object):
	def __init__(self, id, point,width=None):
		self.id = id
		self.point = point
		self.width = width
		self.is_del=False
	...
	def distance(self,other):
		return math.sqrt((self.point.x-other.point.x)*(self.point.x-other.point.x)+(self.point.y-other.point.y)*(self.point.y-other.point.y))

class Edge(object):
	def __init__(self, id, src, dst,width=None):
		self.id = id
		self.src = src
		self.dst = dst
		self.is_del=False
	...
	def get_direction(self):
		return math.atan2((self.dst.point.y-self.src.point.y),(self.dst.point.x-self.src.point.x))

	def get_k(self):
		k=0
		if self.dst.point.x-self.src.point.x==0:
			k=None
		else:
	...
	def is_junction_edge(self):
		if self.src.in_edges>1 or self.src.out_edges>1 or self.dst.in_edges>1 or self.dst.out_edges>1:
			return True
		else:
			return False
	...
	def get_length(self):
		length=math.sqrt((self.src.point.y-self.dst.point.y)*(self.src.point.y-self.dst.point.y)+(self.src.point.x-self.dst.point.x)*(self.src.point.x-self.dst.point.x))
		return length
...
class Graph(object):
	...
	def add_vertex(self, point,width=None):
		vertex = Vertex(len(self.vertices), point,width)
		self.vertices.append(vertex)
		return vertex

	def add_edge(self, src, dst,width=None):
		if src == dst:
			raise Exception('cannot add edge between same vertex')
		edge = Edge(len(self.edges), src, dst,width)
		self.edges.append(edge)
		src.out_edges.append(edge)
		dst.in_edges.append(edge)
		if dst.id not in src.degree:
			src.degree.append(dst.id)
		if src.id not in dst.degree:
			dst.degree.append(src.id)
		return edge
	...
	def save(self, fname):
		with open(fname, 'w') as f:
			for vertex in self.vertices:
				dell=0
				if vertex.is_del==True:
					dell=1
				f.write("{} {} {}\n".format(vertex.point.x, vertex.point.y,dell))
			f.write("\n")
			for edge in self.edges:
				if edge.is_del==True:
					continue
	...
	with open(fname, 'r') as f:
		...
		for line in f:
			...
			if vertex_section:
				...
				ss=int(parts[0])
				dd=int(parts[1])
				if [dd,ss] in edge_list:
					continue
				edge_list.append([ss,dd])
...
def read_pointwidth_graph(fname,merge_duplicates=False):
	graph = Graph()
	with open(fname, 'r') as f:
		vertex_section = True
		vertices = {}
			...
			if vertex_section:
				if len(parts) >= 2:
					...
					if point in seen_points and merge_duplicates:
						...
						vertex = graph.add_vertex(point,width)
						vertices[next_vertex_id] = vertex
						seen_points[point] = vertex
					next_vertex_id += 1
				else:
...
def read_width_graph(fname, merge_duplicates=False):
	graph = Graph()
	with open(fname, 'r') as f:
		vertex_section = True
		vertices = {}
