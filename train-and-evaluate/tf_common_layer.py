...
def glorot(shape, name=None):
	...
	#retrn tf.Variable(initial, name=name)
...
def GraphConvolution(input_tensor,graph_structure,input_dim,output_dim,name=''):
	state_in = tf.layers.dense(inputs=input_tensor, units=output_dim)
	aggregate_result = dot(graph_structure, input_tensor, sparse = True)
	forward_input=tf.layers.dense(inputs=aggregate_result, units=output_dim)

	return state_in+forward_input

def GraphResConvolution(input_tensor,graph_structure,input_dim,output_dim,name=''):
	output_1=GraphConvolution(input_tensor,graph_structure,input_dim,output_dim)
	output_1_relu=tf.nn.relu(output_1)
	output_2=GraphConvolution(output_1_relu,graph_structure,input_dim,output_dim)
	output_2_res=output_2+input_tensor
	output=tf.nn.relu(output_2_res)
	return output
...
def create_gcn_layer_GRU_generic_one_fc(name, input_tensor, graph_structures, input_dim, output_dim, dropout = None, activation = tf.nn.leaky_relu, reuse=True):
	...
	with tf.variable_scope(name + 'fc_vars', reuse=tf.AUTO_REUSE): #1#update feature using neighbors'feature
		...
		for i in xrange(graph_num):
			...
			weights.append(glorot([input_dim, input_dim], name='weights_fc'+str(i)))#128,128
			bias.append(zeros([input_dim], name = 'bias'+str(i)))#128

			print(input_dim*i, input_dim*(i+1))

			#zheli d input_tensor daodi shi what siza

			input_tensor_i = input_tensor[:,input_dim*i:input_dim*(i+1)]  #the ith graph input tensor

			input_tensor_i_ = tf.nn.tanh(dot(input_tensor_i, weights[i]) + bias[i])

			input_tensor_i_ = dot(graph_structures[i], input_tensor_i_, sparse = True)     #n*128

			tensor_list.append(input_tensor_i_)

		input_tensor = tf.concat(tensor_list, axis = 1)   #size:[input_dim * graph_num]

	with tf.variable_scope(name + '_vars', reuse=tf.AUTO_REUSE):#using GRU to update the feature
